...
BRAVE_LOCAL_URL = "https://api.search.brave.com/res/v1/local/search"

async def call_brave_api(url, query, count=10, offset=0):
...
@mcp.tool()
async def brave_web_search(ctx: Context, query: str, count: int = 10, offset: int = 0) -> dict:
    try:
        return await call_brave_api(BRAVE_SEARCH_URL, query, min(count, 20), min(offset, 9))
    except Exception as e:
        return {"error": str(e)}

@mcp.tool()
async def brave_local_search(ctx: Context, query: str, count: int = 10) -> dict:
    try:
        results = await call_brave_api(BRAVE_LOCAL_URL, query, min(count, 20))
        return results if results.get("results") else await brave_web_search(ctx, query, count)
    except Exception as e:
        return {"error": str(e)}
